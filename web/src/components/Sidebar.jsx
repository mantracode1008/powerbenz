...
const Sidebar = () => {
    ...
    return (
        <aside className="fixed inset-y-0 left-0 z-40 bg-white border-r border-slate-200 flex flex-col transition-all duration-300 w-[90px] hover:w-72 group font-inter shadow-xl hover:shadow-2xl overflow-hidden">
            {/* Logo Section */}
            <div className="h-24 flex items-center border-b border-slate-100 whitespace-nowrap overflow-hidden px-6">
                <div className="flex items-center w-full justify-center group-hover:justify-start cursor-pointer transition-all duration-300" onClick={() => window.location.href = '/dashboard'}>
                    {/* Collapsed State: Logo Badge */}
                    <div className="shrink-0 group-hover:hidden flex items-center justify-center w-10 h-10">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-6 space-y-2 scrollbar-hide overflow-x-hidden">
                <div className="px-6 mb-2 text-xs font-bold text-slate-400 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">Main Menu</div>
                {navItems.map((item) => {
                    if (user?.role !== 'Admin') {
